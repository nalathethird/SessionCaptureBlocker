...
public class SessionCaptureBlocker : ResoniteMod
{
    ...
    public override string Link => "https://github.com/nalathethird/SessionCaptureBlocker";

    [AutoRegisterConfigKey]
    private static readonly ModConfigurationKey<bool> enabled =
        new ModConfigurationKey<bool>("enabled", "Enable the mod", () => true);
    ...
    class SessionThumbnailData_ShouldCapture_Patch
    {
        static bool Prefix(SessionThumbnailData __instance)
        {
            if (Config == null)
            {
                Error("Config is null! Allowing capture by default.");
                return true;
            }
            ...
            if (world == null)
            {
                Error("__instance.World is null! Allowing capture.");
                return true;
            }
    ...
    class SessionThumbnailData_StartUpload_Patch
    {
        static bool Prefix(SessionThumbnailData __instance)
        {
            Msg("StartUpload Prefix entered.");

            if (Config == null)
            {
                Error("Config is null! Allowing upload.");
                return true;
            }
            ...
            Msg($"Config - enabled={modEnabled}, private={allowPrivate}, contacts={allowContacts}, contactsPlus={allowContactsPlus}, registeredUsers={allowRegisteredUsers}, lan={allowLAN}, public={allowPublic}, local={allowLocal}");

            if (!modEnabled)
            {
                Msg("Mod is disabled, allowing upload.");
                return true;
            }
            ...
            if (world == null)
            {
                Error("__instance.World is null! Allowing upload.");
                return true;
            }
            ...
            {
                Msg($"World '{world.Name}' is not focused (Focus={world.Focus}), allowing upload.");
                return true;
            }

            var accessLevel = world.AccessLevel;
            Msg($"World '{world.Name}' (AccessLevel={accessLevel}, Focused={world.Focus == World.WorldFocus.Focused})");
            ...
            switch (accessLevel)
            {
                ...
                case SessionAccessLevel.Anyone:
                    ...
                    break;
                default:
                    Msg("Unknown session type, allowing upload.");
                    break;
            }
